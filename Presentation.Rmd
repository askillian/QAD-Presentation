...
# Introduction & Fundamental Definitions
...
For a pair of continuous random variables $X,Y$ with a known joint distribution, the marginal probability density function $p_X$ can be defined as:
...
A copula is the joint distribution of two -- on $[0,1]$ -- continuous random variables, restricted to $[0,1]^2$
...
## Empirical Copula
We can try a similar approach with copulae, since Sklar's theorem (at least) guarantees the existence of a copula $A_n$, i.e. 
...
where $H_n$ is the empirical joint distribution function of the sample.
...
However, depending on the metric used, we will not get convergence to the copula of the underlying distribution.
...
## Checkerbord Copulae: Motivation

Depending on the metric used, an empirical copula might not suffice to estimate the "true" one.

If we take the uniform distance $d_\infty$, then an empirical copula $A_n^B$ (we denote the copula $B$ which we used to construct the empirical) of, say, the product copula $\Pi$ may converge, i.e.
$$ d_\infty(A_n^B, \Pi) = \text{max}\{|A_n^B(x,y) - \Pi(x,y)|; \; x,y \in [0,1]\} \to 0 $$
as $n \to \infty$


## Checkerboard Copulae: Motivation

However, if we use another metric on Copulae, $D_1$, we may be in trouble. To construct this metric, we use special markov kernels called *regular conditional distributions* like this:
$$ K(X(\omega), B) = \mathbb{E}(\mathbb{1}_B \circ Y | X) $$
and then define
$$ D_1(A,B) := \int_{[0,1]} \int_{[0,1]} |K_A(x,[0,y]) - K_B(x,[0,y])| \;dx \; dy $$
In this metric then, $D_1(A_n^B,\Pi) \xrightarrow{n \to \infty} \frac{1}{3}$, which is bad.


## Checkerbord Copulae:
We would like to have convergence even in the $D_1$ metric however.

One problem of empirical copulae could be, that the sample size dictates, how many squares actually have mass and that mass is always the same, $\frac{1}{n}\cdot\text{mass of }B$ or $0$.
...
An absolutely continuous copula, which has a constant density on the interior of each square of our (refined) $N \times N$ grid, we call $N-$ Checkerboard Copula.
...
The function emp_c_copula computes the mass-distribution of the empirical (checkerboard) copula, given a bivariate sample $(x_1,y_1),\dots,(x_n,y_n)$.
...
<!-- TODO: documentation, we can also plot the mass distr -->
The function plot_density allows us to visualize copulae by plotting its density or mass distribution.
...
<!-- TODO: Is this an actual exercise or an draft? I wouldn't understand what to do -->
Try to calculate the empirical copula -- not the checkerboard one -- while having the argument *smoothing=TRUE* on. Why does this work in this particular case? 

Try to explain by also plotting the pseudo observations of your generated sample into the mass-plot (also testing your *ggplot* skill a bit here).
...
plot_density(emp_check_cop, density = FALSE)
```
<!-- TODO: Bei solchen Aufgaben bitte die Lösungen ausformulieren -->
...
$Y$.  
If they are independent, their joint distribution is described by the product copula $\Pi$.

On the other hand we could have complete dependence, i.e. knowing $X$ we
also know $Y$ and vice versa. 

Such a property should be reflected in the copula and we want to use this to construct a *dependency measure*.


## The $\zeta_1$-Measure

Based on the metric $D_1$ such a dependency measure for a copula $A$ can be
constructed in the following way:
$$ \zeta_1(A) := 3\cdot D_1(A,\Pi) \; \in [0,1] $$
where $\Pi$ is, again, the product copula.
...
<!-- Now we also see, why using checkerbard copulae is important: We said before, that for the empircal copulae $A_n^B$ of $\Pi$, we have $D_1(A_n^B,\Pi) \to \infty$. That would mean, that for large $n$, we would estimate -->
<!-- $\zeta_1(A_n^B)\approx 1$, so a high degree of dependency for what should essentially be the product copula (complete independence). -->


## What do we do with that?

Now we calculate the empirical checkerboard copula $A_n$. 
Then we simply estimate the dependency between
$X$ and $Y$ as $\zeta_1(A_n)$

It would be a bit of work to do all that by hand. Luckily there is a
R-function that will help us out.

#QAD - Quantification of Asymmetric Dependence

## The qad object:
A qad object can be generated with: 
```{r, eval=FALSE}
qad(x)
```
where $x$ is a dataframe containing the observations of two univariate/one bivariate sample.

Interesting for us is the qad component *results*, which carries the results of the $\zeta_1$ dependency measure.
...
y = abs(x)
df = data.frame(x,y)
model = qad(df, print = FALSE, permutation = TRUE, resolution = 15)
model$results
```
<!-- Comment: Ich wuerd hier ein Bsp zur ein/beidseitigen Abhaengigkeit einfuegen und dann pairwise/heatmap vorziehen-->


## Exercise 2 - qad

If we look at the output of qad, we see different scores for the degree of dependency of $Y$ on $X$ and vice versa.

Why could that be the case? Try to come up with an example of your own to try and explain this *interesting* bit of information.

<!-- Comment: Ich wuerd hier den plot-Teil + BSP zuerst machen, dann pairwise/heatmap und dann erst das RTR Bsp, weil die 2 Fkt. dafuer sehr hilfreich sein koennten-->
...
y = abs(x)
df = data.frame(x,y)
model = qad(df, print = FALSE, permutation = TRUE, resolution = 10)
```  


## plot.qad
```{r}
plot(model, addSample = TRUE)
```


## predict.qad
```{r}
predict(model, values=c(0.2), conditioned = "x1", nr_intervals = 4)
```
Here we calculate the probability that for $(x_1,x_2)\sim(X,Y)$, $x_2$ lies in each of the intervals above, 
given that $x_1$ lies in the strip that contains $x_1=0.2$.  
<!--Arguments: qad object; evaluating values; conditioned: variable that is the condition; nr_intervals: -->
...
```{r, fig.height = 3}
heatmap.qad(model, select = "dependence", 
                 significance = TRUE, 
                 sign.level = 0.01)
```  
...
We can use the qad metric to test hypothesis:

cci (conditional confidence interval) provides a confidence interval for independence (for the qad measure). 

Therefore we can compare the qad-value to the interval boundaries and check if the null hypothesis of independence holds or has to be rejected.
...
#plot(sample)
model = qad(sample, print = FALSE)
c = cci(n, alternative = "one.sided")[2]
if(coef(model, select = c('q(x1,x2)')) > c){
 print(paste('q(x1,x2)','>',c,sep=' '))
 print('Reject H0')
}else{
 print(paste('q(x1,x2)','<=',c,sep=' '))
 print('Accept H0')
...
```
```{r,echo=TRUE}
cci(100, alternative = "one.sided")
```

## Exercise 3 - pairwise/heatmap
  
  * Create a sample (size = 200) with the following properties:
  ```{r, eval=FALSE}
  n = 200
  x = runif(n,0,1)
  y1 = (x*5)-floor(x*5)
  ...
  ```
  * Use the pairwise and heatmap methods to visualize the dependencies in the sample
  
  * Which pair has the highest asymmetry?
...
## Exercise 4 - qad
  ...
  * Use summary.qad() to get a closer look at the object
  ...
  ```{r, eval=FALSE, echo = FALSE}
    ...
    qad(df$rtr_speed_dl,df$lat)
  
  ```
  
## Ecercise 5 - qad

  * Use the pairwise and heatmap methods to visualize the dependencies in the RTR data set
  
  * Which pair has the highest asymmetry?
  
  * What problems occur?
  ```{r, eval=FALSE, echo = FALSE}
    heatmap.qad(pairwise.qad(df))
