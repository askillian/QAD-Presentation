...
If we take the uniform distance $d_\infty$, then an empirical coupla $A_n^B$ (we denote the Copula $B$ which we used to construct the empirical) of, say, the product copula $\Pi$ may converge, i.e.
...
One problem of empirical copulae could be, that the sample size dictates, how many squares actually have mass and that mass is always the same, $\frac{1}{n}\text{mass of }B$ or $0$.
...
Based on the metric $D1$ such a dependency meassure for a copula A can be
...
Now we also see, why using checkerbard copulae is important: We said before, that for the empircal copulae $A_n^B$ of $\Pi$, we have $ D_1(A_n^B,\Pi) \to \infty$. That would mean, that for large $n$, we would estimate $ \zeta_1(A_n^B) \approx 1 $, so a high degree of dependency for what should essentially be the product copula (complete independence).
...
If we look at the output of qad, we see a different score for the degree of dependency of $Y$ on $X$ and vice versa.
